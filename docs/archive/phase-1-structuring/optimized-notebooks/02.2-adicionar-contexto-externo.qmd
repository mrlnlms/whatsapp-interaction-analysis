...
::: {.callout-warning}
### 💡 Insight - Validação da Hipótese

[PREENCHER APÓS RODAR - resumir se gaps coincidem, redução de volume, significância]
:::
...
::: {.callout-warning}
### 💡 Insight - Impacto nos Padrões Temporais

[PREENCHER APÓS RODAR - resumir se hora/dia mudam quando juntos]
:::
...
- Total de encontros: {len(encontros_df)}
- Dias juntos no período: {dias_juntos_no_dataset} ({pct_juntos:.1f}%)
- Viagem mais longa: {encontros_df['duracao_dias'].max()} dias

**Validação da hipótese:**
- Volume médio separados: {fora_encontros.mean():.1f} msgs/dia
- Volume médio juntos: {durante_encontros.mean():.1f} msgs/dia
- Redução: {reducao_pct:.1f}% (p < 0.001)
- Gaps coincidem com encontros: [VALOR]%
...
**Impacto nos padrões:**
- Padrão horário: [MUDA/NÃO MUDA] quando juntos
- Padrão semanal: [MUDA/NÃO MUDA] quando juntos

**Dataset exportado:** `messages_with_context.parquet` ({df.shape[0]:,} × {df.shape[1]} colunas)

**Próximo passo:** Análise de conteúdo e interação com contexto integrado
:::
