...
-   ⚠️ Não aparece em **todas** as mensagens de mídia, editadas ou excluídas

-   ⚠️ Pode aparecer em outros contextos além desses

-   ⚠️ Confiar exclusivamente nele pode levar a erros de classificação
...
-   `00000001-PHOTO-2025-08-21-22-06-35.jpg` → tipo `PHOTO`

-   `-0000015-AUDIO-2025-08-21-21-31-39.opus` → tipo `AUDIO`

-   `00000298-STICKER-2025-08-25-09-10-40.webp` → tipo `STICKER`

-   `00001238-VIDEO-2025-09-03-13-50-07.mp4` → tipo `VIDEO`

-   `-0001788-Lu mãe.vcf` → tipo `CONTACT` (VCF)
...
|-----------------|-----------------|-----------------|-----------------------|
| `timestamp` | datetime | Data e hora da mensagem | ISO 8601 format |
| `sender` | string | Remetente | Nome do contato |
| `message` | string | Conteúdo da mensagem | Texto da mensagem |
...
| `is_continuation` | boolean | É linha de continuação? | `True`, `False` |
...
`is_continuation` não deveria estar nessa base, pois o tratamento deveria agregar as linhas de continuação na mensagem principal.
...
1.  **Leitura do arquivo bruto**
    ...
    -   Registrar tamanho do arquivo físico do arquivo no sistema
    -   Exibir essas informações basicas.
...
-   Gráfico de barras comparativo: Tamanho e linhas (raw vs. cleaned)

-   Gráfico de pizza: Distribuição de tipos de mensagem (cleaned)

-   Gráfico de barras: Mensagens por remetente

-   Gráfico de linhas: Mensagens ao longo do tempo
...
-   [ ] Remover ou manter no `cleaned-data.txt`? -\> Remover
-   [ ] Usar como marcador durante processamento e remover no final? -\> Sim
-   **Recomendação:** Usar durante processamento, remover no arquivo final

### 3.2 Mensagens Editadas

-   [ ] Manter marcador `[message-edited]` no arquivo limpo? -\> No primeiro, mantem, no segundo retira, mantendo só o conteúdo da mensagem
-   [ ] Preservar texto original ou apenas versão editada? -\> Não tem como preservar original, isso é só uma marcação, apenas. Vira uma coluna acho no dataset, no max.
-   **Recomendação:** Manter marcador, preservar apenas texto final editado, mas só no primeiro arquivo exportado do raw-data, no ultimo, o ultra-clean vc retira o marcador, mantendo apenas o texto.

### 3.3 Anonimização

-   [ ] Manter nomes originais dos remetentes?-\> Por enquanto sim
-   [ ] Substituir por `P1`, `P2` para privacidade? -Sim, deixar fácil essa possibilidade
-   **Recomendação:** Avaliar necessidade de compartilhamento público -\> sim, por isso deixar fácil, para eu testar vendo quem é quem e com caracteress ruins e tals, depois só trocar se quiser.

### 3.4 Fuso Horário

-   [ ] Manter timezone local (America/Sao_Paulo)? -\> Sim
-   [ ] Converter para UTC? -\> Não sei a diferença.. entre utc, são paulo.. Mas acho bom manter o local.
-   **Recomendação:** Manter local com offset explícito: `-03:00 -> sim.`

### 3.5 Mensagens de Sistema

-   [ ] Incluir no `cleaned-data.txt`? - Sim, ele fica com todos marcadores, sabe?
-   [ ] Excluir completamente? -\> Apenas no ultra-cleaned.
-   
...
1.  [x] Revisar e validar decisões conceituais (Seção 1)
2.  [x] Definir respostas para questões em aberto (Seção 3)
