"""
Análise de Sentimento - DistilBERT Multilingual

Modelo: lxyuan/distilbert-base-multilingual-cased-sentiments-student
Classes: 3 (positive, neutral, negative)
Características: Modelo leve (66M params), 2x mais rápido que RoBERTa
...
Output: data/processed/messages_with_models.parquet (atualizado)
        data/processed/sentiment_distilbert_metadata.json

Colunas adicionadas:
    - sentimento_distilbert_label (category): positive/neutral/negative
    - sentimento_distilbert_score (float): confiança do modelo (0-1)
"""
...
def aplicar_sentimento(input_file, analyzer, is_gpu):
    ...
    # 3. ESTIMA TEMPO
    msgs_por_seg = 1000 if is_gpu else 100  # DistilBERT é 2x mais rápido!
    tempo_min = len(df_validas) / msgs_por_seg / 60
    print(f"\n⏱️  Tempo estimado: ~{tempo_min:.1f} minutos")
    print(f"⚡ DistilBERT é ~2x mais rápido que RoBERTa!")
    ...
    df_trabalho['sentimento_distilbert_label'] = None
    df_trabalho['sentimento_distilbert_score'] = None
    
    df_trabalho.loc[mask, 'sentimento_distilbert_label'] = labels
    df_trabalho.loc[mask, 'sentimento_distilbert_score'] = scores
    
    # Converte para category
    df_trabalho['sentimento_distilbert_label'] = df_trabalho['sentimento_distilbert_label'].astype('category')
    
    # 7. RETORNA DF LEVE + TEMPO
    return df_trabalho[['sentimento_distilbert_label', 'sentimento_distilbert_score']], len(df_trabalho), tempo_proc
...
def exibir_resumo(df):
    ...
    total = df['sentimento_distilbert_label'].notna().sum()
    print(f"\n✅ Analisadas: {total:,}")
    
    if total > 0:
        print(f"\n📈 Distribuição:")
        dist = df['sentimento_distilbert_label'].value_counts()
        ...
        for label in ['positive', 'neutral', 'negative']:
            if label in df['sentimento_distilbert_label'].values:
                score = df[df['sentimento_distilbert_label'] == label]['sentimento_distilbert_score'].mean()
...
def salvar_metadata(df, tempo_total, tempo_proc, device_name):
    ...
    metadata = {
        ...
        'mensagens_processadas': int(df['sentimento_distilbert_label'].notna().sum()),
        'mensagens_puladas': int(df['sentimento_distilbert_label'].isna().sum()),
        'batch_size': BATCH_SIZE,
        'device': device_name,
        'distribuicao': {
            str(k): int(v) for k, v in df['sentimento_distilbert_label'].value_counts().to_dict().items()
        },
        'confianca_media': {
            label: float(df[df['sentimento_distilbert_label'] == label]['sentimento_distilbert_score'].mean())
            for label in ['positive', 'neutral', 'negative']
            if label in df['sentimento_distilbert_label'].values
        }
    }
...
def main():
    print("="*80)
    print("⚡ SENTIMENT ANALYSIS - DistilBERT Multilingual")
    print("="*80)
    ...
    df_full['sentimento_distilbert_label'] = df_sentiment['sentimento_distilbert_label']
    df_full['sentimento_distilbert_score'] = df_sentiment['sentimento_distilbert_score']
    ...
    # Comparação com RoBERTa (se existir)
    if 'sentimento_roberta_label' in df_full.columns:
        print("\n" + "="*80)
        print("🔍 COMPARAÇÃO RÁPIDA: DistilBERT vs RoBERTa")
        print("="*80)
        
        # Mensagens com ambos os modelos
        ambos = df_full[
            df_full['sentimento_distilbert_label'].notna() & 
            df_full['sentimento_roberta_label'].notna()
        ]
        
        if len(ambos) > 0:
            concordancia = (ambos['sentimento_distilbert_label'] == ambos['sentimento_roberta_label']).mean()
            print(f"\n✅ Concordância: {concordancia*100:.1f}%")
            
            print("\n📊 Distribuição comparada:")
            print("   DistilBERT:", dict(df_full['sentimento_distilbert_label'].value_counts()))
            print("   RoBERTa:   ", dict(df_full['sentimento_roberta_label'].value_counts()))
    ...
    print(f"\n💡 Próximo passo:")
    print(f"   Renderize o notebook para ver a comparação!")
    print(f"   quarto render notebooks/04-model-features.qmd")
